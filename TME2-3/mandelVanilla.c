...
int main(int argc, char *argv[]) {
  ...
  if (rang==0)
    {
      for (j=0; j<w*h/nombre; j++) {pima[j]=ima[j];}
      for (i=1;i<nombre;i++) {MPI_Recv(&pima[i*w*h/nombre],w*h/nombre,MPI_CHAR,i ,tag, MPI_COMM_WORLD,NULL);}
    }
  else
    {
      MPI_Send(ima,w*h/nombre,MPI_CHAR,0,tag, MPI_COMM_WORLD);
      MPI_Finalize();
      return 0;
    }
  ...
  sauver_rasterfile( "mandelVanilla.ras", w, h, pima);
