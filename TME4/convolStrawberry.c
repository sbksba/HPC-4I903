...
int main(int argc, char *argv[]) {
  ...
  unsigned char* img = (unsigned char*) malloc (w * hloc * sizeof(unsigned char));

  MPI_Scatter(r.data,w*h/N,MPI_CHAR,img,w*h/N,MPI_CHAR,0,MPI_COMM_WORLD);


  for (i=0;i<nbiter; i++){
    if (k !=0){
      MPI_Send(img + w,w, MPI_CHAR, k-1,tag,MPI_COMM_WORLD);
    }
    if (k != N-1){
      MPI_Recv(img+ (hloc-1)*w,w,MPI_CHAR,k+1,tag,MPI_COMM_WORLD, NULL);
      MPI_Send(img+ (hloc-2)*w,w,MPI_CHAR,k+1,tag,MPI_COMM_WORLD);
    }


    if (k!=0){
      MPI_Recv(img,w,MPI_CHAR,k-1,tag,MPI_COMM_WORLD, NULL);
    }

    convolution(filtre,img,hloc,w);

  }

  MPI_Gather(img, w*h/N, MPI_CHAR,r.data,w*h/N,MPI_CHAR,0,MPI_COMM_WORLD);
